...
DisassemblerGraphViewOld::DisassemblerGraphViewOld(QWidget* parent)
...
DisassemblerGraphViewOld::~DisassemblerGraphViewOld()
{
    delete this->highlight_token;
}

void DisassemblerGraphViewOld::initFont()
{
...
void DisassemblerGraphViewOld::adjustSize(int width, int height)
{
...
void DisassemblerGraphViewOld::resizeEvent(QResizeEvent* event)
{
    adjustSize(event->size().width(), event->size().height());
}

duint DisassemblerGraphViewOld::get_cursor_pos()
{
    if(this->cur_instr == 0)
...
void DisassemblerGraphViewOld::set_cursor_pos(duint addr)
{
...
std::tuple<duint, duint> DisassemblerGraphViewOld::get_selection_range()
{
    return std::make_tuple<duint, duint>(get_cursor_pos(), get_cursor_pos());
}

void DisassemblerGraphViewOld::set_selection_range(std::tuple<duint, duint> range)
{
    this->set_cursor_pos(std::get<0>(range));
}

void DisassemblerGraphViewOld::copy_address()
{
...
void DisassemblerGraphViewOld::paintNormal(QPainter & p, QRect & viewportRect, int xofs, int yofs)
{
...
void DisassemblerGraphViewOld::paintOverview(QPainter & p, QRect & viewportRect, int xofs, int yofs)
{
...
void DisassemblerGraphViewOld::paintEvent(QPaintEvent* event)
{
    Q_UNUSED(event);
...
bool DisassemblerGraphViewOld::isMouseEventInBlock(QMouseEvent* event)
{
...
duint DisassemblerGraphViewOld::getInstrForMouseEvent(QMouseEvent* event)
{
    //Convert coordinates to system used in blocks
    int xofs = this->horizontalScrollBar()->value();
    int yofs = this->verticalScrollBar()->value();
...
bool DisassemblerGraphViewOld::getTokenForMouseEvent(QMouseEvent* event, Token & tokenOut)
{
    /* TODO
    //Convert coordinates to system used in blocks
    int xofs = this->horizontalScrollBar()->value();
...
bool DisassemblerGraphViewOld::find_instr(duint addr, Instr & instrOut)
{
    for(auto & blockIt : this->blocks)
...
void DisassemblerGraphViewOld::mousePressEvent(QMouseEvent* event)
{
    if(!DbgIsDebugging())
        return;
    if(drawOverview)
...
void DisassemblerGraphViewOld::mouseMoveEvent(QMouseEvent* event)
{
    if(this->scroll_mode)
    {
...
void DisassemblerGraphViewOld::mouseReleaseEvent(QMouseEvent* event)
{
...
void DisassemblerGraphViewOld::mouseDoubleClickEvent(QMouseEvent* event)
{
    if(drawOverview)
    {
        toggleOverviewSlot();
...
void DisassemblerGraphViewOld::prepareGraphNode(DisassemblerBlock & block)
{
    int width = 0;
    int height = 0;
    for(auto & line : block.block.header_text.lines)
...
void DisassemblerGraphViewOld::adjustGraphLayout(DisassemblerBlock & block, int col, int row)
{
    block.col += col;
    block.row += row;
...
void DisassemblerGraphViewOld::computeGraphLayout(DisassemblerBlock & block)
{
...
bool DisassemblerGraphViewOld::isEdgeMarked(EdgesVector & edges, int row, int col, int index)
{
...
void DisassemblerGraphViewOld::markEdge(EdgesVector & edges, int row, int col, int index, bool used)
{
...
int DisassemblerGraphViewOld::findHorizEdgeIndex(EdgesVector & edges, int row, int min_col, int max_col)
{
    //Find a valid index
    int i = 0;
    while(true)
...
int DisassemblerGraphViewOld::findVertEdgeIndex(EdgesVector & edges, int col, int min_row, int max_row)
{
    //Find a valid index
    int i = 0;
    while(true)
...
DisassemblerGraphViewOld::DisassemblerEdge DisassemblerGraphViewOld::routeEdge(EdgesVector & horiz_edges, EdgesVector & vert_edges, Matrix<bool> & edge_valid, DisassemblerBlock & start, DisassemblerBlock & end, QColor color)
{
...
void DisassemblerGraphViewOld::renderFunction(Function & func)
{
...
void DisassemblerGraphViewOld::show_cur_instr(bool force)
{
    for(auto & blockIt : this->blocks)
    {
        DisassemblerBlock & block = blockIt.second;
...
bool DisassemblerGraphViewOld::navigate(duint addr)
{
    //Add address to history
    if(!mHistoryLock)
...
void DisassemblerGraphViewOld::fontChanged()
{
    this->initFont();
...
void DisassemblerGraphViewOld::setGraphLayout(DisassemblerGraphViewOld::LayoutType layout)
{
...
void DisassemblerGraphViewOld::tokenizerConfigUpdatedSlot()
{
    disasm.UpdateConfig();
    loadCurrentGraph();
}

void DisassemblerGraphViewOld::loadCurrentGraph()
{
...
void DisassemblerGraphViewOld::loadGraphSlot(BridgeCFGraphList* graphList, duint addr)
{
...
void DisassemblerGraphViewOld::graphAtSlot(duint addr)
{
    Bridge::getBridge()->setResult(this->navigate(addr) ? this->currentGraph.entryPoint : 0);
}

void DisassemblerGraphViewOld::updateGraphSlot()
{
    this->viewport()->update();
}

void DisassemblerGraphViewOld::addReferenceAction(QMenu* menu, duint addr)
{
...
void DisassemblerGraphViewOld::setupContextMenu()
{
...
void DisassemblerGraphViewOld::keyPressEvent(QKeyEvent* event)
{
...
void DisassemblerGraphViewOld::followDisassemblerSlot()
{
    DbgCmdExec(QString("disasm %1").arg(ToPtrString(this->cur_instr)).toUtf8().constData());
}

void DisassemblerGraphViewOld::colorsUpdatedSlot()
{
...
void DisassemblerGraphViewOld::fontsUpdatedSlot()
{
    fontChanged();
}

void DisassemblerGraphViewOld::shortcutsUpdatedSlot()
{
    updateShortcuts();
}

void DisassemblerGraphViewOld::toggleOverviewSlot()
{
...
void DisassemblerGraphViewOld::toggleSummarySlot()
{
    drawOverview = false;
...
void DisassemblerGraphViewOld::selectionGetSlot(SELECTIONDATA* selection)
{
    selection->start = selection->end = cur_instr;
    Bridge::getBridge()->setResult(1);
}

void DisassemblerGraphViewOld::disassembleAtSlot(dsint va, dsint cip)
{
    Q_UNUSED(va);
...
void DisassemblerGraphViewOld::gotoExpressionSlot()
{
    if(!DbgIsDebugging())
        return;
    if(!mGoto)
...
void DisassemblerGraphViewOld::gotoOriginSlot()
{
    DbgCmdExec("graph cip, silent");
}

void DisassemblerGraphViewOld::gotoPreviousSlot()
{
...
void DisassemblerGraphViewOld::gotoNextSlot()
{
...
void DisassemblerGraphViewOld::toggleSyncOriginSlot()
{
...
void DisassemblerGraphViewOld::refreshSlot()
{
    DbgCmdExec(QString("graph %1, force").arg(ToPtrString(this->cur_instr)).toUtf8().constData());
}

void DisassemblerGraphViewOld::saveImageSlot()
{
    saveGraph = true;
    this->viewport()->update();
}

void DisassemblerGraphViewOld::setCommentSlot()
{
...
void DisassemblerGraphViewOld::setLabelSlot()
{
    duint wVA = this->get_cursor_pos();
    LineEditDialog mLineEdit(this);
...
void DisassemblerGraphViewOld::xrefSlot()
{
...
void DisassemblerGraphViewOld::decompileSlot()
{
...
void DisassemblerGraphViewOld::followActionSlot()
{
