...
//                text += '.';
//        }
        text = Config()->mDocCodec->toUnicode(data);
        break;
    ...
    case Encoding::Utf8:
        text = QTextCodec::codecForName("UTF-8")->makeDecoder(QTextCodec::IgnoreHeader)->toUnicode(data);
        break;
    }
    text.replace(QChar::ReplacementCharacter, QChar('.')).replace(QChar('\0'), QChar('.'));
...
//            data.append(text[i].toLatin1());
	...
	case Encoding::Utf8:
		data = QTextCodec::codecForName("UTF-8")->makeEncoder(QTextCodec::IgnoreHeader)->fromUnicode(text);
		break;
    }
