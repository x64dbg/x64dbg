...
9. Load/unload a DLL during a fast-path run. It must rebuild the snapshot without stepping or calling the trace callback from the loader context. Verify initial entry into the newly loaded module is observed. Only a failed refresh should roll back and switch that traversal to stepping.
