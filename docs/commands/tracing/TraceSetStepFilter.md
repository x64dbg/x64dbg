...
* Windows-managed CFG/SCP image-extension trampolines are omitted from the temporary breakpoint set because Windows rejects protection changes on them. Running excluded code may pass through these trampolines without a trace callback even if their party matches. This exception is limited to the single-page `ImageExtension` layout recognized by the memory map (see [RunToParty](RunToParty.md)); normal stepping and ordinary private/JIT coverage are unchanged.
* A DLL load/unload rebuilds the snapshot from the updated module map and continues running. It does not itself invoke a trace callback. If refreshing fails, newly installed breakpoints are rolled back and that traversal uses the original stepping mode until the selected party is reached; the Log reports the failure. Successful refreshes are logged too.
* Enabled user memory breakpoints, guard pages, missing target pages, or initial setup failures prevent the fast path. Partial setup is rolled back and the remainder of the trace uses stepping instead.
