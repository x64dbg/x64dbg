...
* Windows-managed CFG/SCP image-extension trampolines are omitted from the temporary breakpoint set because Windows rejects protection changes on them. Running excluded code may pass through these trampolines without a trace callback even if their party matches. This exception is limited to OS-reported CFG/SCP extensions; normal stepping and ordinary private/JIT coverage are unchanged.
